...
export class AuthStrategy extends PassportStrategy(Strategy) {
     ...
     */
    async validate(payload: { type: string, id: string }) {
        if (!payload) {
        ...
        if (user.isBlock) throw new ApiException('账号已被停用', ApiErrorCode.ACCOUNT_INVALID, 406);
        if (user.isDelete) throw new ApiException('账号已删除', ApiErrorCode.ACCOUNT_INVALID, 406);
        return user;
    }
}